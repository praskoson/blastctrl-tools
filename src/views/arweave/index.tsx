...
export const UploaderView = ({ irys }: { irys: IrysStorage }) => {
  ...
  const handleDrop = (event: DragEvent<HTMLFormElement | HTMLDivElement>) => {
    ...
    if (event.dataTransfer.files && event.dataTransfer.files[0]) {
      ...
      setFile(droppedFile);
      irys.getUploadPrice(droppedFile.size).then((price) => setFilePrice(price));
    }
  };
  ...
  const handleSetFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target?.files?.length === 1) {
      ...
      irys.getUploadPrice(selectedFile.size).then((price) => setFilePrice(price));
    }
  };
            ...
            <div className="overflow-hidden bg-white shadow sm:rounded-lg">
              ...
              <div className="border-t border-gray-200 bg-gray-50">
                ...
                >
                  ...
                  >
                    {isUploading && (
                      <SpinnerIcon className="-ml-1 mr-3 h-5 w-5 animate-spin" aria-hidden="true" />
                    )}
        ...
        <div className="mx-auto my-3 max-w-sm rounded-md border border-gray-300 px-3 py-2 shadow-sm">
          ...
          <div className="my-3 w-full">
            ...
            <p className="pt-2 text-xs text-gray-500">
              These are the leftovers due to how the pricing works. Minimum balance for withdrawing
              is 5000 lamports.
            </p>
          </div>
        </div>
