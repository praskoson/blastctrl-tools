...
export const sameCurrencies = (left: Currency | Amount, right: Currency | Amount): boolean => {
  if ("currency" in left) {
    left = left.currency;
  }
...
export function assertCurrency<T extends Currency>(
  ...
  if (!sameCurrencies(actual, expected)) {
    ...
    throw Error(`Unexpected currency, actual: ${actual} expected: ${expected}`);
  }
}
...
export function assertSol(actual: Currency | Amount): asserts actual is SolCurrency | SolAmount;
export function assertSol(actual: Currency | Amount): asserts actual is SolCurrency | SolAmount {
...
export const addAmounts = <T extends Currency>(left: Amount<T>, right: Amount<T>): Amount<T> => {
...
export const divideAmount = <T extends Currency>(left: Amount<T>, divisor: number): Amount<T> => {
...
export const isEqualToAmount = <T extends Currency>(left: Amount<T>, right: Amount<T>): boolean =>
  compareAmounts(left, right) === 0;

export const isLessThanAmount = <T extends Currency>(left: Amount<T>, right: Amount<T>): boolean =>
  compareAmounts(left, right) < 0;
